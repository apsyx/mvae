...
def load_cifar10(
    ...
    return train_x.astype('float32'), train_y, test_x.astype('float32'), test_y, normalizer

def load_imagenet32(
        dataset=_get_datafolder_path()+'/imagenet32',
        normalize=True,
        dequantify=True):
    ...
    '''
    datasetfolder = os.path.dirname(dataset)
    assert os.path.exists(datasetfolder), 'ImageNet 32 dataset not exist!'

    from torchvision.datasets import ImageFolder
    from torchvision.transforms import ToTensor
    
    train_x = ImageFolder(root=dataset + '/train', transform=ToTensor())
    test_x = ImageFolder(root=dataset + '/valid', transform=ToTensor())
    print len(train_x)
    print len(test_x)
